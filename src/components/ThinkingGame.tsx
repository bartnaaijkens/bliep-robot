...
export function ThinkingGame({ session, phase, onAnswer, c, bg }: Props) {
  const active = phase === 'thinking-question'
  const showResult = phase === 'thinking-correct' || phase === 'thinking-wrong'

  // During the feedback phase currentIndex has NOT been incremented yet, so
  // q always refers to the question that was just answered — correct behaviour.
  const q = session.questions[Math.min(session.currentIndex, session.questions.length - 1)]
  const total = session.questions.length

  // Advance the progress bar as soon as the answer is submitted.
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
      ...
      {/* Question label for odd-one-out */}
      {q.type === 'odd-one-out' && (
        <div style={{
          fontFamily: '"Patrick Hand", cursive', fontSize: 20,
          color: c.deepBlue, opacity: 0.75, textAlign: 'center',
        }}>
          Welke hoort er niet bij?
        </div>
      )}

      {/* Question prompt (pattern / true-false / deduction) */}
      {q.type !== 'odd-one-out' && (
        <div style={{
          fontFamily: '"Patrick Hand", cursive',
          fontSize: q.type === 'deduction' ? 17 : 22,
        ...
        {q.options.map((option, i) => {
          ...
          return (
            <button
              ...
              disabled={!active}
              style={optionButtonStyle(c, bg, active, isCorrectOption, isWrongChoice, isTrueFalse)}
            >
              {option}
            </button>
          )
